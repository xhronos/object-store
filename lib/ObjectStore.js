...
class ObjectStore {
	...
	*/
	...
	async delete(path=[]) {
		path = splitPath(path);
		if (!path || !path.length) throw new ObjStoreError(`delete needs a path`);
		...
		const obj = this.store.get(id);
		// may throw
		const updated = await this.onDelete(id, obj, path);
		if (path.length === 0) {
			// delete whole objects
			this.store.delete(id, obj);
			await this.idProvider.free(id);
			return;
		}
		else {
			// delete object part
